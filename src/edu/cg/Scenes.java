...
public class Scenes {
	...
	public static Scene scene6() {
		...
		Material plainMaterial = Material.getMetalMaterial();
		Surface plainSurface = new Surface(plainShape,plainMaterial);
		Light dirLight = new DirectionalLight().initDirection(new Vec(0.0, -3.0, -1.0))
				.initIntensity(new Vec(0.5));
		Light pointLight = new PointLight().initIntensity(new Vec(0.4)).
				initPosition(new Point(1.5, 0.0, 1.0)).initDecayFactors(0.1, 0.089, 1.0);
		Light spotLight = new Spotlight().initIntensity(new Vec(0.7)).
				initPosition(new Point(-3.5, 0.0, 1.0)).initDecayFactors(0.1, 0.05, 1.0)
				.initDirection(new Vec(-0.5,-0.5,-0.5));
		
		return new Scene().initAmbient(new Vec(0.0))
				.initCamera(new Point(0.0, 3.0, 1.0), new Vec(1.5, -0.8, -0.2), new Vec(1.5, -0.5, 0.5), 2.0)
				.addLightSource(dirLight).addLightSource(pointLight).addLightSource(spotLight)
				.addSurface(plainSurface).initName("scene6").initAntiAliasingFactor(1)
				.initRenderRefarctions(true).initRenderReflections(true).initMaxRecursionLevel(3);
	}
	...
	public static Scene scene7() {
		...
		Shape sphereShape5 = new Sphere(new Point(0.9, 0.9, 0.9), 0.2);
		Material sphereMat5 = Material.getRandomMaterial();
		Surface sphereSurface5 = new Surface(sphereShape5, sphereMat5);
		...
		return new Scene().initAmbient(new Vec(1.0))
				.initCamera(new Point(0.0, 0.0, 2.0), new Vec(0.0, 0.0 , -1.0),
						new Vec(1.0, 1.0, 0.0), 1.0)
				.addLightSource(dirLight).addSurface(sphereSurface1).addSurface(sphereSurface2)
				.addSurface(sphereSurface3).addSurface(sphereSurface4).addSurface(sphereSurface5).initName("scene7").initAntiAliasingFactor(1)
				.initRenderRefarctions(true).initRenderReflections(true).initMaxRecursionLevel(6);
	}
